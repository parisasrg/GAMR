...
public class LineRendererComponent : MonoBehaviour
{
    ...
    public void LineRendererComponentFn(float hue, GameObject playerPrefab){
        ...
        if(replaySystem.loadLineAtOnce)
        {
            ...
            {
                lineRenderer.SetPosition(i, new Vector3(replaySystem.load_positions[i].x, -0.2f, replaySystem.load_positions[i].z));
            }
        }
        else
        {
    ...
    {
        lr.positionCount = i;
        Vector3 pos = new Vector3(load_positions[i].x, -0.2f, load_positions[i].z);
