        ...
        foreach(string file in files)
        {
            ...
            if(audioLoadDatas != null)
            {
                for(int cnt = 0; cnt < audioLoadDatas.Count; cnt++)
                {
                    if(cnt == 0)
                    {
                        GameObject audiodata = Instantiate(audioDataPrefab, new Vector3(audioLoadDatas[cnt].audioPosition.x, -0.2f, audioLoadDatas[cnt].audioPosition.z), audioLoadDatas[cnt].audioRotation);
